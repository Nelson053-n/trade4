...
ST4_PAIRS: dict[str, tuple] = {
    # 04.07: обе торгующие пары переведены на exit_mode=Band (SAR band-to-band) — на
    # тарифе %-от-нотионала классический выход на SMA не окупает комиссию (средняя
    # сделка +39₽ против издержек круга ~30₽); Band забирает всю ширину канала
    # (бэктест 10м/60д: sber +1455₽/лот ср.+112, sngr BB(200,3σ) +5169₽/лот ср.+1723).
    "sber": ("SBRF", "SBPR", "Сбербанк", {"exit_mode": "Band"}),   # BB(200)/2σ дефолт
    # Сургут для Band-режима: широкий канал BB(200, 3σ) — редкие развороты канала целиком
    # (прежний узкий туннель σ1.5/sma60 был откалиброван под выход на SMA и фикс-комиссию).
    "sngr": ("SNGR", "SNGP", "Сургутнефтегаз", {
        "sma_period": 200, "sigma_multiplier": 3.0, "deviation_mode": "Sigma",
        "deviation_sigma": 2.5, "stop_sigma": 4.5, "entry_trigger": "Breakout",
        "exit_mode": "Band"}),
    ...
    # сделки префа агрегируются в непрерывный ряд.
    # ⚠ Бэктест 2026-06-25 (120 дней): даже на 60m набирается лишь ~174 бара (преф крайне
    # неликвиден) → 0 сделок при ЛЮБЫХ параметрах. Пара нежизнеспособна на текущих данных,
    # держим в расчёте на появление ликвидности RTKMP на FORTS. Торгует = 0.
    ...
    # синхронизации на 60m (167 общих баров из 195 за 20 дней).
    # ✓ Бэктест 2026-06-25 (120 дней, 378 баров): sma100/σ2.0/devS2.0 → +4027₽, win 100%,
    # maxDD 1.2% — ЛУЧШИЙ результат среди всех пар. Калибровка подтверждена: укорочение окна
    # (sma60/σ1.5) ломает пару (−1625₽). Текущие пороги оптимальны, не трогать.
