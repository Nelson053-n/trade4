...
class StrategyConfig(BaseModel):
    ...
    # режим выхода: Mean — пересечение средней (§9.4, классика); Band — «band-to-band»
    # (SAR): выход при пересечении ПРОТИВОПОЛОЖНОЙ полосы с немедленным реверсом —
    # забираем всю ширину канала, сделок меньше, средняя сделка кратно жирнее комиссии
    # (бэктест 04.07: sber ср. +112₽/лот vs +39₽ у Mean). Полосы живые, freeze/тейк
    # к Band не применяются.
    exit_mode: Literal["Mean", "Band"] = "Mean"
