...
⚠️ post_order здесь тратит РЕАЛЬНЫЕ деньги. Гейт ордера (executor._post): mode==tbank_real
И real_trading_armed (+cooldown 600с) — для ВСЕХ ордеров. Дополнительно ВХОД в новую позицию
гейтится trading_enabled на уровне портфеля (can_open); выходы/flat от trading_enabled НЕ
зависят (чтобы позиция не залипла при выключенной торговле).
"""
