...
class St5StrategyConfig(BaseModel):
    ...
    # режим выхода: mean — |z| < z_exit_full (классика, выход у середины); band —
    # «band-to-band» (SAR, как st4 exit_mode=Band): SHORT от +z_entry закрывается
    # только при проходе z ДО −z_entry (и наоборот) — забираем весь канал, сделок
    # меньше, средняя кратно жирнее %-комиссии. Реверс не требует спец-кода: вход
    # st5 по УРОВНЮ |z|>z_entry — на следующем баре откроется противоположная позиция.
    # Частичная фиксация в band-режиме не используется; стопы работают как прежде.
    exit_mode: str = "mean"
