...
# Префы без собственной страницы на smart-lab: их дивиденды лежат на странице ОБЫЧКИ
# отдельными строками (парсер отбирает по колонке тикера). Проверено 12.08:
# /q/TATNP/ и /q/LSNGP/ → 404, но /q/SNGSP/ → 200. Поэтому маппинг ТОЧЕЧНЫЙ:
# «отрезать P у любого префа» сломало бы работающие тикеры.
_PREF_PAGE = {"TATNP": "TATN", "LSNGP": "LSNG"}
...
class St8Session:
    ...
    def _smartlab_divs(tk: str) -> list[tuple[str, float]]:
        ...
        ПРЕФЫ (аудит 12.08): у части привилегированных бумаг своей страницы НЕТ —
        `/q/TATNP/` и `/q/LSNGP/` отдают 404, а их дивиденды лежат на странице
        ОБЫЧКИ (`/q/TATN/`, `/q/LSNG/`) отдельными строками. Правило не общее:
        SNGSP свою страницу имеет (HTTP 200). Поэтому маппинг точечный, а не
        «отрезать P у любого тикера» — иначе сломались бы работающие."""
        page = _PREF_PAGE.get(tk, tk)
        url = f"https://smart-lab.ru/q/{page}/dividend/"
